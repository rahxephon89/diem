...
impl<'a> Instrumenter<'a> {
    fn run(
        ...
        if auto_trace {
            ...
            mem_info = usage_analysis::get_memory_usage(&FunctionTarget::new(fun_env, &builder.data)).accessed.get_all_inst(&builder.data.type_args);
        }
    ...
    fn emit_traces(&mut self, spec: &TranslatedSpec, emitted_exp: &ExpData) {
        ...
        for memory in self.mem_info {
            let env = self.builder.global_env();
            let node_id = env.new_node(env.unknown_loc(), memory.to_type());
            self.builder.emit_with(|id| {
                Call(
                    id,
                    vec![],
                    Operation::TraceGlobalMem(memory.clone(), node_id),
                    vec![],
                    None,
                )
            });
