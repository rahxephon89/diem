...
use move_model::{
    ...
    model::{FunId, FunctionVisibility, GlobalEnv, QualifiedId},
...
fn print_script_reach(env: &GlobalEnv) {
    ...
    if func_ids.is_empty() {
        println!("warning: no function is reached from the script function");
    } else {
